 ...
 * @param {string} callBackForFirstInit - callback при инициализации записи о счетчике.
 ...
 */
async function getDataCounters(addressId: string, counterName: string, callbackForFirstInit?: (obj: TypeCounterInfo) => void): Promise<TypeCounterInfo | undefined> {
    try {
        ...
        if (obj) {
            ...
            if (savedObj) {
                ...
                if (callbackForFirstInit) {
                    callbackForFirstInit(objCurrent);
                }
                return objCurrent;
            }
        }
...
* }
*/
async function getDataAndNearestReadingCounter(addressId: string, counterName: string, callbackForFirstInit?: (obj:TypeCounterInfo) => void): Promise<{ counterInfo: TypeCounterInfo | undefined, nearestReading: TypeCounterMeters | undefined }> {
    try {
        // Получение данных о счетчике
        const counterInfo = await getDataCounters(addressId, counterName, callbackForFirstInit);
