...
const GasCounterScreen = () => {
    ...
    const [currentGasReading, setCurrentGasReading] = useState<TypeCounterMeters>();
    //Лоадер
    ...
    async function saveData(data: { inputOne?: string, inputTwo?: string }) {
        try {
            ...
            if (data && data.inputOne && gasCurrent) {
                await saveReading(data.inputOne, gasCurrent);
            }
        }
        catch (err) {
            console.log(err);
    ...
    * }
    */
    async function getDataGasCounter() {
        try {
            // Получение данных о счетчиках горячей и холодной воды
            const gas = await getDataAndNearestReadingCounter(address.id.toString(), 'Газ',
                async (counterData) => {
            ...
            if (gas && gas.counterInfo) {
                setGasCurrent(gas.counterInfo);
            }

            // Получение ближайших показаний счетчиков горячей и холодной воды
            if (gas.nearestReading) {
                setCurrentGasReading(gas.nearestReading);
            }
    ...
    return (
        gasCurrent ?
            <ScreenCounter
                ...
                currentReading={currentGasReading}
